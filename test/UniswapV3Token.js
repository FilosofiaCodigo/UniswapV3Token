...
describe("UniswapV3Token", function () {
  // We define a fixture to reuse the same setup in every test.
  // We use loadFixture to run this setup once, snapshot that state,
  // and reset Hardhat Network to that snapshot in every test.
  async function deployFixture() {
    const [deployer, user1, user2, vaultWallet] = await ethers.getSigners();

    const V3Fees = await ethers.getContractFactory("V3Fees");
    const v3Fees = await V3Fees.deploy(
      "V3 Fees",            // Name
      "V3F",                // Symbol
      vaultWallet.address,  // Vault wallet
      "100",                // 1.00% P2P fee
      "200",                // 2.00% Sell fee
      "1000");              // Rate: 1000 tokens ~= 1 eth
    ...
    return { v3Fees, nonfungiblePositionManager, router, weth, deployer, user1, user2 };
  }

  describe("Fee tests", function () {
    it("Buy, Sell and P2P Fees", async function () {
      const { v3Fees, nonfungiblePositionManager, router, weth, deployer, user1, user2 } = await loadFixture(deployFixture);
